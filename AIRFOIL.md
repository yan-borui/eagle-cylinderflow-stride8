...
原生GraphViT结构与6帧训练窗、1000epochs、常数LR1e-4保持。四卡各B1、global B4。真实Airfoil标签2只在模型输入中映射到槽6，区分原生padding槽2；归档仍保留官方标签。boundary_channels为空，所有真实节点的UVP/q自由预测，padding单独清零。
训练seed123、FP32。配置为`cylinderflow_config_4gpu.json`，其内容和数据身份已切换到Airfoil；
保留模块/文件名用于复用现有原生训练和恢复路径。

```bash
export PREPARED_DIR=/shared/artifacts/airfoil_eagle
export RESULT_ROOT=/shared/runs/airfoil_eagle_seed123
