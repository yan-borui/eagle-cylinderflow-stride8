...
class GraphViT(nn.Module):
    ...
    def forward(
        ...
        Existing callers retain their original boundary behavior. The matched
        CylinderFlow adapter supplies first-frame values and velocity channels.
        """
        if apply_noise:
        ...
        for t in range(1, state.shape[1]):
            ...
            if boundary_values is None:
                ...
                channels = (
                    ...
                    else boundary_channels
                )
                for channel in channels:
                    next_state[..., channel] = torch.where(
                        mask, boundary_values[..., channel], next_state[..., channel]
                    )
                disabled = node_type[:, t, :, NODE_DISABLE] == 1
                next_state = torch.where(
                    disabled[..., None], torch.zeros_like(next_state), next_state
                )
        ...
        if return_boundary_raw:
            return velocity_hat, output_hat, target, torch.stack(raw_states, dim=1)
...
class AttentionBlock_PreLN(nn.Module):
    ...
    def forward(self, W, attention_mask, posenc):
        ...
        W2 = self.attention(W1_posenc, W1_posenc, W1_posenc, attn_mask=attention_mask)[
            0
        ]
        W3 = W + self.linear(W2)
...
class GraphPooling(nn.Module):
    def __init__(self, w_size, pos_length):
        ...
        self.linear_rnn = MLP(
            input_size=w_size, output_size=w_size, n_hidden=1, layer_norm=False
        )

    def forward(self, V, clusters, positional_encoding, cluster_mask):
        pos_by_cluster = torch.gather(
            positional_encoding,
            -2,
            clusters.reshape(clusters.shape[0], -1, 1).repeat(
...
class Encoder(nn.Module):
    def __init__(self, nb_gn=4, state_size=3, pos_length=7):
        ...
        self.encoder_node = MLP(
            input_size=9 + state_size, output_size=128, n_hidden=1, layer_norm=False
        )
        self.encoder_edge = MLP(
            input_size=3, output_size=128, n_hidden=1, layer_norm=False
        )
...
class Positional_Encoder(nn.Module):
    ...
    def forward(self, mesh_pos, clusters, cluster_mask):
        ...
        relative_positions = pos_embeddings.scatter(
            -2,
            clusters.reshape(B, -1, 1).repeat(1, 1, S),
