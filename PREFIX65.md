...
Airfoil 的 EAGLE 使用 stored frames 0–64 训练，覆盖前 64 个时间间隔。
...
首次准备数据可先运行 `bash scripts/airfoil_4gpu.sh prepare`，然后将 PREPARED 指向生成的 prepared_eagle_4gpu 目录。
...
export RESULT_ROOT=/absolute/path/to/airfoil_eagle_prefix65
